...
class TripsDao extends DatabaseAccessor<AppDatabase> with _$TripsDaoMixin {
  ...
  Future<List<TripsTableData>> getAll() => (select(
    tripsTable,
  ...
  Future<List<TripsTableData>> getOngoing() {
    ...
    return (select(tripsTable)..where(
          (t) => t.status.equals('ongoing') & t.date.isSmallerOrEqualValue(now),
        ))
        .get();
  }
  ...
  Future<void> updateStatus(String id, String status) async {
    await (update(tripsTable)..where((t) => t.id.equals(id))).write(
      TripsTableCompanion(status: Value(status)),
    );
  }
