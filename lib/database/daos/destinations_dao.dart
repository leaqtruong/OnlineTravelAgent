...
class DestinationsDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<DestinationsTableData>> getRecommended() => (select(
    destinationsTable,
  )..where((t) => t.isRecommended.equals(true))).get();

  Future<List<DestinationsTableData>> getByCategory(String category) => (select(
    destinationsTable,
  )..where((t) => t.category.equals(category))).get();

  Future<List<DestinationsTableData>> search(String query) => (select(
    destinationsTable,
  )..where((t) => t.name.like('%$query%') | t.location.like('%$query%'))).get();

  Future<DestinationsTableData?> getById(String id) => (select(
    destinationsTable,
  ...
  Future<void> toggleFavorite(String id) async {
    ...
    if (dest != null) {
      await (update(destinationsTable)..where((t) => t.id.equals(id))).write(
        DestinationsTableCompanion(isFavorite: Value(!dest.isFavorite)),
      );
    }
  }
  ...
  Future<void> setFavorite(String id, bool isFavorite) async {
    await (update(destinationsTable)..where((t) => t.id.equals(id))).write(
      DestinationsTableCompanion(isFavorite: Value(isFavorite)),
    );
  }
}
