...
class SyncService {
  ...
  Future<void> _syncDestinations(
      List<Destination> destinations, List<Destination> recommended) async {
    final recommendedIds = recommended.map((d) => d.id).toSet();

    await db.destinationsDao.insertAll(
      destinations.map((d) => DestinationsTableCompanion.insert(
            id: d.id,
            name: d.name,
            location: d.location,
  ...
  Future<void> _syncHotels(List<Hotel> hotels) async {
    ...
    for (final h in hotels) {
      hotelCompanions.add(HotelsTableCompanion.insert(
        id: h.id,
        name: h.name,
        location: h.location,
      ...
      for (final r in h.rooms) {
        roomCompanions.add(RoomsTableCompanion.insert(
          id: r.id,
          hotelId: h.id,
          name: r.name,
  ...
  Future<void> _syncTours(List<TourPackage> tours) async {
    await db.tourPackagesDao.insertAll(
      tours.map((t) => TourPackagesTableCompanion.insert(
            id: t.id,
            name: t.name,
  ...
  Future<void> _syncDocuments(List<DocumentItem> documents) async {
    await db.documentsDao.insertAll(
      documents.map((d) => DocumentsTableCompanion.insert(
            id: d.id,
            title: d.title,
            description: Value(d.description),
  ...
  Future<void> _syncTrips(List<Trip> trips) async {
    await db.tripsDao.insertAll(
      trips.map((t) => TripsTableCompanion.insert(
            id: t.id,
  ...
  Future<void> _syncSchedule(String tripId, TripSchedule schedule) async {
    ...
    for (final day in schedule.days) {
      dayCompanions.add(TripScheduleDaysTableCompanion.insert(
        id: day.id,
        tripId: tripId,
    ...
    for (final update in schedule.updates) {
      updateCompanions.add(TripScheduleUpdatesTableCompanion.insert(
        id: update.id,
        tripId: tripId,
  ...
  Future<BootstrapData> loadBootstrapFromSQLite() async {
    ...
    final destinations = destRows.map((d) => Destination(
      id: d.id, name: d.name, location: d.location,
      rating: d.rating, duration: d.duration, imagePath: d.imagePath,
      isFavorite: d.isFavorite, description: d.description,
      price: d.price, reviewsCount: d.reviewsCount,
      category: d.category, latitude: d.latitude, longitude: d.longitude,
    )).toList();

    final recommended = recommendedRows.map((d) => Destination(
      id: d.id, name: d.name, location: d.location,
      rating: d.rating, duration: d.duration, imagePath: d.imagePath,
    )).toList();
    ...
    final hotels = hotelRows.map((h) {
      return Hotel(
        ...
        rating: h.rating, imagePath: h.imagePath,
        description: h.description, priceFrom: h.priceFrom,
        address: h.address,
        amenities: List<String>.from(jsonDecode(h.amenities as String? ?? '[]')),
      );
    }).toList();

    final tours = tourRows.map((t) => TourPackage(
      id: t.id, name: t.name, description: t.description,
      imagePath: t.imagePath, duration: t.duration, price: t.price,
      originalPrice: t.originalPrice,
      destinations: List<String>.from(jsonDecode(t.destinations as String? ?? '[]')),
      includes: List<String>.from(jsonDecode(t.includes as String? ?? '[]')),
      departure: t.departure, departureDate: t.departureDate,
      isPopular: t.isPopular, includesGuide: t.includesGuide,
      guideFee: t.guideFee,
    )).toList();

    final trips = tripRows.map((t) => Trip(
      id: t.id, destination: t.destination, location: t.location,
      date: t.date, guests: t.guests, status: t.status,
      imagePath: t.imagePath, isUpcoming: t.isUpcoming,
      flightId: t.flightId, hotelId: t.hotelId, roomId: t.roomId,
      totalPrice: t.totalPrice, isCustom: t.isCustom,
    )).toList();

    final documents = docRows.map((d) => DocumentItem(
